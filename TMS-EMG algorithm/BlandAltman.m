...
addTitle([], baAH, drawAreaPos, params);
...
if strcmpi(params.processCorrelationOnly,'On')
    ...
    addTitle(cAH, [], drawAreaPos, params);
...
if ~strcmpi(params.processCorrelationOnly,'On')
    ...
    if strcmpi(params.Legend,'On')
        addLegend([],baAH,drawAreaPos, params);
    end
    DisplayBAStats(baAH, params, stats,data)
    rpc = stats.rpc;
end
...
zero_1 = find(data.set1 == 0);
zero_2 = find(data.set2 == 0);
indices = intersect(zero_1,zero_2);
data.set1(indices) = 0.001;
data.set2(indices) = 0.001;

data.set1(data.set1==0)=NaN; 
	...
	switch upper(params.corrInfo{i})
		case 'EQ'
			if ~strcmpi(params.forceZeroIntercept,'off')
				corrtext = [corrtext; ['y=' num2str(stats.slope,3) 'x']];
			elseif stats.intercept>=0
				corrtext = [corrtext; ['y=' mynum2str(stats.slope,3) 'x+' num2str(stats.intercept,3)]];
			else
				corrtext = [corrtext; ['y=' mynum2str(stats.slope,3) 'x-' num2str(stats.intercept,3)]];
			end
		case 'R2', corrtext = [corrtext; ['r^2=' num2str(stats.r^2,3)]];
		case 'R', corrtext = [corrtext; ['r=' num2str(stats.r,3)]];
		case 'P', corrtext = [corrtext; ['p=' num2str(stats.corrP,3)]];
		case 'RHO', corrtext = [corrtext; ['rho=' mynum2str(stats.rho,4,4)]];
		case 'RHO (P)', corrtext = [corrtext; ['rho=' num2str(stats.rho,3) ' (p=' num2str(stats.rhoP,3) ')']];
		case 'SSE', corrtext = [corrtext; ['SSE=' num2str(stats.SSE,3) params.unitsStr]];
		case 'RMSE', corrtext = [corrtext; ['RMSE=' num2str(stats.RMSE,4) params.unitsStr]];
...
a = axis(ah);
ch = get(ah,'children');
set(findobj(ch, 'flat', 'tag','CorrInfoText'),'Position', [a(1)+0.1 a(4)]);
...
%stats.differenceMedianP = signrank(data.set1,data.set2); %MODIFICADO DO CÓDIGO ORIGINAL
...
if strcmpi(params.baYLimMode,'Squared') 
	%a = [params.axesLimits(1) params.axesLimits(2) stats.differenceMedian+stats.rpcNP+(stats.differenceMedian+stats.rpcNP)*0.5 (stats.differenceMedian-stats.rpcNP)-(stats.differenceMedian-stats.rpcNP)*0.5];
    %a = [params.axesLimits(1:2) [-1 1]*abs(params.axesLimits(2)-params.axesLimits(1))/2];
    %axis(baAH,a);
else
	a = axis(baAH);
    %a = [params.axesLimits(1:2) stats.differenceMedian+stats.rpcNP+(stats.differenceMedian+stats.rpcNP)*0.5 (stats.differenceMedian-stats.rpcNP)-(stats.differenceMedian-stats.rpcNP)*0.5];
    %axis(baAH, a);
end
...
switch upper(params.baStatsMode)
	case {'NORMAL','GAUSSIAN'}
		plot(baAH,a(1:2),stats.differenceMean+[0 0],'k','MarkerSize',5)
		plot(baAH,a(1:2),stats.differenceMean+stats.rpc*[1 1],'--k','MarkerSize',5)
		plot(baAH,a(1:2),stats.differenceMean-stats.rpc*[1 1],'--k','MarkerSize',5)
        plot(baAH,a(1:2),[0 0],'--m','MarkerSize',5);
	...
	case 'NON-PARAMETRIC'
		plot(baAH,a(1:2),stats.differenceMedian+[0 0],'k','MarkerSize',5)
		plot(baAH,a(1:2),stats.differenceMedian+stats.rpcNP*[1 1],'--k','MarkerSize',5)
		plot(baAH,a(1:2),stats.differenceMedian-stats.rpcNP*[1 1],'--k','MarkerSize',5)
        plot(baAH,a(1:2),[0 0],'--m','MarkerSize',5);
        
		text(a(2),stats.differenceMedian+stats.rpcNP, ['LOA = ',num2str(stats.differenceMedian+stats.rpcNP,6), ' (bias + 1.45IQR)'],'Parent',baAH,'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'Tag','ULimLabel');
		text(a(2),stats.differenceMedian,['bias = ',num2str(stats.differenceMedian,6),' [p-value Wilconx =',mynum2str(stats.differenceMedianP,2,4), ']'],'Parent',baAH,'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'Tag','MeanLabel');
		text(a(2),stats.differenceMedian-stats.rpcNP, ['LOA = ',num2str(stats.differenceMedian-stats.rpcNP,6), ' (bias - 1.45IQR)'],'Parent',baAH,'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'Tag','LLimLabel');
		%%
...
function DisplayBAStats(baAH, params, stats,data)
BAtext = {};
for i=1:length(params.baInfo)
	switch upper(params.baInfo{i})
		...
		%case 'RPCNP', BAtext = [BAtext; ['{\bfLOA: ' mynum2str(stats.rpcNP,2) params.diffUnitsStr '}']];
...
a = gca;
a.YLim = [stats.differenceMedian-stats.rpcNP-(stats.differenceMedian-stats.rpcNP)*1.5,(stats.differenceMedian+stats.rpcNP)+(stats.differenceMedian+stats.rpcNP)*1.5];
%a = axis(baAH);
a1 = a.XLim
a2 = a.YLim
text(a1(2),a2(2),BAtext,'interpreter','tex','HorizontalAlignment','left','VerticalAlignment','bottom','Parent',baAH,'tag','BAInfoText');
...
%% Add legend to the plot
function addLegend(cAH,baAH, drawAreaPos, params)
...
if ~strcmpi(params.symbols,'Num') && ~isempty(gnames)
    if strcmpi(params.processCorrelationOnly,'On')
        ...
        units = get(fig,'Units');
        set(fig,'Units','Normalized');
        %set(get(get(ph(7),'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
        %set(get(get(ph(8),'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
        set(m,'String',gnames);
        ...
        set(m,'Position',[drawAreaPos(1)+(drawAreaPos(3)-width)/2 drawAreaPos(2) width drawAreaPos(4)*0.05]);
        set(fig,'Units',units);
    else
        lh = legend(baAH,'show');
        if iscell(gnames)
